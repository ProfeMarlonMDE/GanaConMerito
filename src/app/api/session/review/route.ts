...
export async function GET(request: Request) {
  ...
  const value = url.searchParams.get("attemptId");
  const sessionId = url.searchParams.get("sessionId");
  if (value && !z.string().uuid().safeParse(value).success) return NextResponse.json({error:"Invalid attempt"},{status:400});
  if (sessionId && !z.string().uuid().safeParse(sessionId).success) return NextResponse.json({error:"Invalid session"},{status:400});
